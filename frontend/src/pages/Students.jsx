...
import { Plus, Upload, Search, UserPlus, X, Users, UserMinus, FolderPlus, Trash2, RotateCcw, UserX } from 'lucide-react';
...
export default function Students() {
  ...
  const getDisplayed = () => {
    ...
    if (tab === 'my') list = myStudents;
      ...
      const g = groups.find(g => g.id === parseInt(tab));
      return g ? [] : [];
    }
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit flex-wrap">
        {[
          ...
          ...groups.map(g => ({ key: String(g.id), label: `${g.name} (${g.student_count})` })),
        ...
        <button onClick={() => setShowGroupCreate(true)}
          className="px-3 py-2 text-sm text-gray-400 hover:text-gray-600 rounded-md">
                    ...
                    <div className="flex gap-1 justify-end">
                      ...
                      {tab === 'inactive' && (
                        <button onClick={() => api.setStudentActive(s.id).then(load)}
                          className="p-1.5 rounded hover:bg-green-50 text-gray-400 hover:text-green-600" title="Reactivate">
